    ...
    def forward(
        ...
        for downsample_block in self.down_blocks:
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                # image_encoder_hidden_states was passed through to these blocks unconditionally upstream,
                # but they're the *stock* diffusers CrossAttnDownBlock2D/UNetMidBlock2DCrossAttn (imported
                # directly, not a custom subclass) and their forward() signatures have never accepted this
                # kwarg — this checkpoint has use_image_cross_attention=False (confirmed via its own config,
                # logged at load time), so the value was always None here; dropped rather than plumbed
                # through, since there's no compatible stock block to receive it even if it weren't.
