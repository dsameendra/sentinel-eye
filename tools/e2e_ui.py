...
async def main():
    async with Browser() as b:
        ...
        # A click must never pause the picture: it stays un-paused and time keeps moving (allow a few seconds:
        # the rig's synthetic HD source is transcoded on a busy machine and can hiccup on its own)
        moved = await b.wait_for(f"document.querySelector('.focus video').currentTime > {t0} + 0.8", 8)
        paused = await b.js("document.querySelector('.focus video').paused")
        check("clicking the video does NOT pause it", moved and paused is False, f"paused={paused} kept playing={moved}")
